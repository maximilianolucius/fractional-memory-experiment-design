...
def _find(*candidates):
    """Locate an artifact in either the reproducibility package layout (data/...)
    or the development repository layout (rescue_compute/...)."""
    for c in candidates:
...
S2 = json.load(open(_find("data/safe_design_search/r3f_stage2_summary.json",
                          "rescue_compute/r3_safe_design_search/r3f_stage2_summary.json")))
VS = json.load(open(_find("data/r3d_validated_safety.json",
                          "rescue_compute/r3d_validated_safety.json")))
PW = json.load(open(_find("data/r3d_pwc6.json",
                          "rescue_compute/r3d_pwc6.json")))
...
outdir = os.path.join(os.path.dirname(ROOT), "figures")
if not os.path.isdir(outdir):
    outdir = os.path.join(ROOT, "paper/figures")
out = os.path.join(outdir, "fig24_safe_design_frontier.pdf")
