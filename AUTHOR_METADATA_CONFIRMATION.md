...
Three of four items are now resolved. **One blocker remains: the funding statement.**
...
## 3. **BLOCKER — data deposit identifier**
...
- [ ] Funding: state Option 1 or Option 2 (+ funder and grant number)
- [ ] Upload `public_reproducibility/` to Zenodo, obtain the DOI
- [ ] Decide (a) / (b) / (c) on the existing preprint record and its creator list
- [ ] Confirm the phone number may be entered in Editorial Manager
- [ ] Confirm the postal address line is the one the author wants printed
